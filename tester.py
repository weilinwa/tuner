...
def run_benchmark_embed(model, token_comb, containers_conf, qpc, is_warmup, it=1):
    ...
    if is_warmup:
        docker_command = f"docker run -it --cpuset-cpus={cpus} --rm --net=host {PROXY_ENV} -v {BENCHMARK_DIR_BASE}/benchmarks/:/workspace/vllm/benchmarks/ -v {BENCHMARK_DIR_SCRIPTS_BASE}/benchmark_serving_v0.9.1.py:/workspace/vllm/benchmarks/benchmark_serving_embedding.py -v {BENCHMARK_DIR_SCRIPTS_BASE}/backend_request_func_v0.9.1.py:/workspace/vllm/benchmarks/backend_request_func.py -v {model_dir}:/root/.cache -e HUGGING_FACE_HUB_TOKEN={HUGGING_FACE_HUB_TOKEN} --entrypoint=python3 {container_image} /workspace/vllm/benchmarks/benchmark_serving_embedding.py --port 8000 --backend vllm-embed --endpoint /v1/embeddings --dataset-name random --request-rate {concurrency} --num-prompts {num_prompts} --random-input-len {inp_tokens} --random-output-len {inp_tokens} --ignore-eos --percentile-metrics ttft,tpot,itl,e2el --served-model-name {served_model_name} --metric-percentiles 50,90,99 --max-concurrency {concurrency} --model {model}"
    else:
        docker_command = f"docker run -it --cpuset-cpus={cpus} --rm --net=host {PROXY_ENV} -v {BENCHMARK_DIR_BASE}/benchmarks/:/workspace/vllm/benchmarks/ -v {BENCHMARK_DIR_SCRIPTS_BASE}/benchmark_serving_v0.9.1.py:/workspace/vllm/benchmarks/benchmark_serving_embedding.py -v {BENCHMARK_DIR_SCRIPTS_BASE}/backend_request_func_v0.9.1.py:/workspace/vllm/benchmarks/backend_request_func.py -v {model_dir}:/root/.cache -v {results_dir}:/results -e HUGGING_FACE_HUB_TOKEN={HUGGING_FACE_HUB_TOKEN} --entrypoint=python3 {container_image} /workspace/vllm/benchmarks/benchmark_serving_embedding.py --port 8000 --backend vllm-embed --endpoint /v1/embeddings --dataset-name random --request-rate {concurrency} --num-prompts {num_prompts} --random-input-len {inp_tokens} --random-output-len {inp_tokens} --ignore-eos --percentile-metrics ttft,tpot,itl,e2el --served-model-name {served_model_name} --metric-percentiles 50,90,99 --max-concurrency {concurrency} --save-result --result-filename {results_file_container} --model {model}"
